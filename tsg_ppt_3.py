...
def fill_slide_common(tsg_ppt, org_all, a, c1, c2, c3, data, box=None):
  common_slide=tsg_ppt.slides[a]
  ...
  chart_data.categories= [c1,c2,c3]
  chart_data.add_series('01', (data[0], data[1], data[2]))
...
def fill_table_slide(tsg_ppt,a,i,n):
...
def fill_slide_common1(tsg_ppt, org_all, a, c1, c2, c3, data, box=None, data_prev1 = None, data_prev2 = None, data_next = None):
  common_slide=tsg_ppt.slides[a]
  mittelwert = common_slide.placeholders[18]
  text_frame = mittelwert.text_frame
  text_frame.clear()
...
structure_3.fill_in_org()
print str(len(org_long)) + " " + str(len(org_short))


#######################
#fillin stuff
level5_users,level5_numbers,level5_filledin_users = structure_3.fill_in_users(structure_3.list5, "u_org_5", structure_3.users_number5)
list_full_5 = (structure_3.printout(structure_3.list5,level5_users))
filled_list_5,my_sums_5,my_means_5 = structure_3.count_percent(list_full_5, level5_numbers)

level4_users,level4_numbers,level4_filledin_users = structure_3.fill_in_users(structure_3.list4, "u_org_4", structure_3.users_number4)
list_full_4 = (structure_3.printout(structure_3.list4,level4_users))
filled_list_4,my_sums_4,my_means_4 = structure_3.count_percent(list_full_4, level4_numbers)

level3_users,level3_numbers,level3_filledin_users = structure_3.fill_in_users(structure_3.list3, "u_org_3", structure_3.users_number3)
list_full_3 = (structure_3.printout(structure_3.list3,level3_users))
filled_list_3,my_sums_3,my_means_3 = structure_3.count_percent(list_full_3, level3_numbers)

level2_users,level2_numbers,level2_filledin_users = structure_3.fill_in_users(structure_3.list2, "u_org_2", structure_3.users_number2)
list_full_2 = (structure_3.printout(structure_3.list2,level2_users))
filled_list_2,my_sums_2,my_means_2 = structure_3.count_percent(list_full_2, level2_numbers)

level1_users,level1_numbers,level1_filledin_users = structure_3.fill_in_users(structure_3.list1, "u_org_1", structure_3.users_number1)
list_full_1 = (structure_3.printout(structure_3.list1,level1_users))
filled_list_1,my_sums_1,my_means_1 = structure_3.count_percent(list_full_1, level1_numbers)

#######################

for for_int,my_organi in enumerate(structure_3.list5):
  if (level5_filledin_users[for_int] < 5):
    print 'not created: '+my_organi
  else:
    tsg_ppt=Presentation('tsg_templ_3.pptx')
    fill_slide_mean(tsg_ppt, my_organi,  2, "Ich gebe meiner Führungskraft Feedback.", "Am Ende des Feedbackgesprächs werden Absprachen getroffen.", "Ich erhalte Feedback zu meinem Beitrag zum Teamerfolg.", "Das Feedback hilft mir, mein Verhalten zu verändern.", "Das Feedbackgespräch baut auf vorherigem Feedback auf.", "Ich erhalte Rückmeldungen zu meiner Gesprächsführung im Kundenkontakt (interner/externer Kunde).", "Wie häufig erhalte ich Rückmeldung zu meiner Leistung von meiner Führungskraft?", my_means_5[for_int])
    fill_table_slide(tsg_ppt, level5_numbers[for_int], level5_filledin_users[for_int], str(round(float(level5_filledin_users[for_int] / float(level5_numbers[for_int]))*100,2)) + '%')
    for i in range(1, 12):
      fill_slide_title(tsg_ppt, i, org_long[org_short.index(my_organi)]) #org_long_name
    for i in range(0, 12):
      if (6 <= i <=12 or i==3):
        ###!!!!! Out of index... other lists index!
        fill_slide_common1(tsg_ppt, org_long[org_short.index(my_organi)], i, "Trifft überhaupt nicht zu / Trifft eher nicht zu", "Teils-teils", "Trifft eher zu / Trifft voll zu", filled_list_5[for_int][i-3],'Mittelwert auf dreistufiger Skala:'+'\n'+str(my_means_5[for_int][i-3])+'\n'+"Gültige Antworten:"+'\n'+str(my_sums_5[for_int][i-3]), filled_list_4[for_int][i-3], filled_list_3[for_int][i-3]) #org_long_name
      elif (i==4):
        fill_slide_not_common(tsg_ppt, org_long[org_short.index(my_organi)], i, "täglich", "maximal 1x pro Woche", "bis zu 1x pro Monat", "halbjährlich", "seltener", filled_list_5[for_int][i-3],"\n"+"Gültige Antworten:"+"\n"+str(my_sums_5[for_int][i-3]))
      elif (i==5):
        fill_slide_not_common_2(tsg_ppt, org_long[org_short.index(my_organi)], i, "1-3 min", "3-5 min", "5-15 min", "15-30 min", "länger", filled_list_5[for_int][i-3],"\n"+"Gültige Antworten:"+"\n"+str(my_sums_5[for_int][i-3]))
    tsg_ppt.save("out1/"+(structure_3.list5[for_int].replace(" ", "_")).replace("/","_")+"_TSG_Leadership_Survey"+".pptx")
##//repeat


for asdf,my_organi in enumerate(structure_3.list4):
  if (level4_filledin_users[asdf] < 5):
    ...
    for i in range(0, 12):
      if (6 <= i <=12 or i==3):
        fill_slide_common(tsg_ppt, org_long[org_short.index(my_organi)], i, "Trifft überhaupt nicht zu / Trifft eher nicht zu", "Teils-teils", "Trifft eher zu / Trifft voll zu", filled_list_4[asdf][i-3],'Mittelwert auf dreistufiger Skala:'+'\n'+str(my_means_4[asdf][i-3])+'\n'+"Gültige Antworten:"+'\n'+str(my_sums_4[asdf][i-3])) #org_long_name
      elif (i==4):
    ...
    tsg_ppt.save("out1/"+(structure_3.list4[asdf].replace(" ", "_")).replace("/","_")+"_TSG_Leadership_Survey"+".pptx")



for asdf,my_organi in enumerate(structure_3.list3):
  if (level3_filledin_users[asdf] < 5):
    ...
    for i in range(0, 12):
      if (6 <= i <=12 or i==3):
        fill_slide_common(tsg_ppt, org_long[org_short.index(my_organi)], i, "Trifft überhaupt nicht zu / Trifft eher nicht zu", "Teils-teils", "Trifft eher zu / Trifft voll zu", filled_list_3[asdf][i-3],'Mittelwert auf dreistufiger Skala:'+'\n'+str(my_means_3[asdf][i-3])+'\n'+"Gültige Antworten:"+'\n'+str(my_sums_3[asdf][i-3])) #org_long_name
      elif (i==4):
    ...
    tsg_ppt.save("out1/"+(structure_3.list3[asdf].replace(" ", "_")).replace("/","_")+"_TSG_Leadership_Survey"+".pptx")
#



for asdf,my_organi in enumerate(structure_3.list2):
  if (level2_filledin_users[asdf] < 5):
    ...
    for i in range(0, 12):
      if (6 <= i <=12 or i==3):
        fill_slide_common(tsg_ppt, org_long[org_short.index(my_organi)], i, "Trifft überhaupt nicht zu / Trifft eher nicht zu", "Teils-teils", "Trifft eher zu / Trifft voll zu", filled_list_2[asdf][i-3],'Mittelwert auf dreistufiger Skala:'+'\n'+str(my_means_2[asdf][i-3])+'\n'+"Gültige Antworten:"+'\n'+str(my_sums_2[asdf][i-3])) #org_long_name
      elif (i==4):
    ...
    tsg_ppt.save("out1/"+(structure_3.list2[asdf].replace(" ", "_")).replace("/","_")+"_TSG_Leadership_Survey"+".pptx")
#


for asdf,my_organi in enumerate(structure_3.list1):
  if (level1_filledin_users[asdf] < 5):
    ...
    for i in range(0, 12):
      if (6 <= i <=12 or i==3):
        fill_slide_common(tsg_ppt, org_long[org_short.index(my_organi)], i, "Trifft überhaupt nicht zu / Trifft eher nicht zu", "Teils-teils", "Trifft eher zu / Trifft voll zu", filled_list_1[asdf][i-3],'Mittelwert auf dreistufiger Skala:'+'\n'+str(my_means_1[asdf][i-3])+'\n'+"Gültige Antworten:"+'\n'+str(my_sums_1[asdf][i-3])) #org_long_name
      elif (i==4):
    ...
    tsg_ppt.save("out1/"+(structure_3.list1[asdf].replace(" ", "_")).replace("/","_")+"_TSG_Leadership_Survey"+".pptx")
#
#-------------
