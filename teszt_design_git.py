...
def fill_slide_1(tsg_ppt, org_names, org_percents):
  ...
  x,y,cx,cy = Inches(0.3), Inches(2.0), Inches(9.38), Inches(4.7)
  ...
  #series.fill.solid()
  #series.fill.fore_color.rgb=RGBColor(0,0,0)
  return tsg_ppt
...
def fill_slide_2(tsg_ppt, q1):
  ...
  x,y,cx,cy = Inches(0.3), Inches(2.00), Inches(5.38), Inches(4.7)
...
def fill_slide_common(tsg_ppt, x):
  common_slide=tsg_ppt.slides[x]
  chart_data = ChartData()
  chart_data.categories=['1']
  a = [30,40,30] 
  b = [20,30, 50]
  c = [20,59.4,20.6]
...
def chartfromarray(tsg_ppt, tomb1, tomb2):
  chart_data = ChartData()
  chart_data.categories=tomb1
  for i in tomb1:
    chart_data.add_series('valami',tomb2[i])
